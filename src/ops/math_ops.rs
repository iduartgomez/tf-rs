...
pub fn floor_div<Tx, Ty, S>(scope: &mut Scope, x: Tx, y: Ty, name: S) -> Result<Tensor>
where
    Tx: TensorOps,
    Ty: TensorOps,
    S: AsRef<Path>,
{
    let x = x.into_tensor(scope);
    let y = y.into_tensor(scope);
    scope.install(FloorDiv::new(x, y, name)?)
}
    ...
    S: AsRef<Path>,
{
    let x = x.into_tensor(scope);
    let y = y.into_tensor(scope);
    scope.install(Maximum::new(x, y, name)?)
}
    ...
    S: AsRef<Path>,
{
    let input = input.into_tensor(scope);
    let axis = axis.into_tensor(scope);
    scope.install(Max::new(input, axis, name)?.keep_dims(&[keep_dims]))
}
