...
type CondSubGraph = Box<FnMut(&mut Scope) -> Result<Vec<Tensor>, ::Error>>;
type WhileCondGraph = Box<FnMut(&mut Scope, &[Tensor]) -> Result<Tensor, ::Error>>;
type WhileBodyGraph = Box<FnMut(&mut Scope, &[Tensor]) -> Result<Vec<Tensor>, ::Error>>;
...
impl CondContextInterface for Scope {
    fn cond_scope<S: AsRef<Path>>(
        &mut self,
        mut cond_context: CondContext,
        name: S,
    ) -> Scope {
        self.allow_writes();
...
pub(crate) trait WhileContextInterface {
    fn loop_scope<S: AsRef<Path>>(
        &mut self,
        cond_context: WhileContext,
        name: S,
    ) -> Scope;
    fn build_loop(
        &mut self,
...
impl WhileContextInterface for Scope {
    ...
    fn build_loop(
        ...
        */
        ...
        let c = pred(self, loop_vars.clone())?;
        ...
        // Add the exit ops.
        let exit_vars =
            switch_vars.iter().map(|&(x, _)| exit(self, x, "")).collect::<Result<Vec<_>, _>>()?;
...
mod test {
    ...
    fn test_cond() {
        use super::assign;
        let mut context = Scope::new();
        let var: Tensor = context.get_variable("", DataType::Int32, Some(&[])).unwrap().into();
    ...
    fn test_while_loop() {
        ...
        let pred = Box::new(
            move |mut scope: &mut Scope, loop_vars: &[Tensor]| {
        ...
        let op = while_loop(&mut context, pred, body, &[x.into()], "").unwrap()[0];
